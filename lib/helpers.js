...
const addUserToGame = (gameId, username) => {
  ...
  .then(() => {
    return redisAsPromised.hmset(username, 'position', '[]', 'letters', '[]', 'gameId', gameId)
  })
  .catch(err => {
    throw err
  })
...
const editUser = (username, letters, position) => {
  return redisAsPromised.hmset(username, 'position', JSON.stringify(position), 'face', 'N', 'letters', JSON.stringify(letters))
  .catch(err => {
    throw err
  })
}
